...
#@REM Script usage: setenv <role> (where role should be: froggergame.server / froggergame.client)
...
#@REM ==== NETWORK STUFF ====
#@REM Must run http froggergame.server on codebase SMTP_HOST_ADDR:
#@REM Python 2.7: python -m SimpleHTTPServer 8000
#@REM Python 3: python -m http.froggergame.server 8000
...
#java.rmi.froggergame.server.codebase property defines the location where the froggergame.client/froggergame.server provides its classes.
